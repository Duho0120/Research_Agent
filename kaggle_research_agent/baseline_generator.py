...
def main() -> int:
    ...
    majority_value = _majority_value(train_rows, args.target)
    ...
    submission_rows = _make_submission_rows(test_rows, sample_rows, submission_columns, id_columns, args.target, majority_value)
    _write_csv(out_dir / "submission.csv", submission_columns, submission_rows)

    metrics = {
        "trial_id": args.trial,
        "cv_score": _training_accuracy(train_rows, args.target, majority_value),
        "lb_score": None,
        "objective": "maximize",
        "model_type": "majority_class_baseline",
        "target_column": args.target,
        "prediction_value": majority_value,
        "train_rows": len(train_rows),
        "test_rows": len(test_rows),
        "notes": "Generated by baseline_train.py. Majority-class baseline for first local sanity check.",
    }
